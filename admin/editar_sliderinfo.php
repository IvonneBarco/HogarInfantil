...
<head>
	...
	<title id="titulo">.:: Editar Información Institucional</title>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE info SET titulo=%s, descripcion=%s, imagen=%s, orden=%s, estado=%s WHERE codigo=%s",
  ...
  $updateGoTo = "lista_sliderinfo.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_cnx_slider, $cnx_slider);
$query_edit_slider = sprintf("SELECT * FROM info WHERE info.codigo = %s", $varDato_edit_slider);
