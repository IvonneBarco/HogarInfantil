...
switch ($_GET["accion"])
{
	case "listar_evento":
	{
		...
		if ($fila=$query->fetch_array())
		{
			do
			{
				echo "<p>".$fila["evento"]."<a href='#' class='eliminar_evento' rel='".$fila["id"]."' title='Eliminar este Evento del ".fecha($_GET["fecha"])."'></a></p>";
			}
	...
	case "generar_calendario":
	{
		...
		echo "<img src='images/logo.png' style='width: 15%'; height: 15%;><br><h2 class='font-luck'>HOGAR INFANTIL LAS MERCEDES<br>Calendario de Eventos<br>".$meses[intval($fecha_calendario[1])]." de ".$fecha_calendario[0]." <abbr title='S&oacute;lo se pueden agregar eventos en d&iacute;as h&aacute;biles y en fechas futuras (o la fecha actual).'></abbr></h2>";
